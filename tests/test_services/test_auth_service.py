...
class TestCreateUnverifiedNewUser:
    ...
    async def test_create_new_user_success(
        self, auth_service, mock_user_repo, mock_intern_repo, mock_code_repo
    ):
        ...
        mock_code_repo.create_code.assert_awaited_once_with(
            conn=ANY, user_id=created_user_mock.id, code=ANY
        )
...
class TestVerifyUser:
    ...
    @patch(
        "src.services.auth_service.generate_access_token",
        return_value="fake.access.token",
    )
    async def test_verify_user_success(
        self, mock_generate_token, auth_service, mock_code_repo, mock_user_repo
    ):
...
class TestLogin:
    ...
    async def test_login_success(
        self, mock_password_check, mock_generate_token, auth_service, mock_user_repo
    ):
        existing_verified_user = create_mock_user(verified=True)
        mock_user_repo.get_user_by_email_or_phone.return_value = existing_verified_user
    ...
    async def test_login_user_not_verified(
        self, mock_password_is_correct, auth_service, mock_user_repo
    ):
...
class TestRequestResetPassword:
    ...
    async def test_request_reset_password_user_exists(
        self, auth_service, mock_user_repo, mock_code_repo, mock_background_tasks
    ):
        existing_user = create_mock_user(verified=True)
        mock_user_repo.get_user_by_email_or_phone.return_value = existing_user
        ...
        mock_user_repo.get_user_by_email_or_phone.assert_awaited_once_with(
            conn=ANY, email=existing_user.email
        )
        mock_code_repo.upsert_code_with_user_id.assert_awaited_once()
        mock_background_tasks.add_task.assert_called_once()
        assert "a password reset email will be sent" in result["detail"]

    async def test_request_reset_password_user_does_not_exist(
        self, auth_service, mock_user_repo, mock_code_repo, mock_background_tasks
    ):
        mock_user_repo.get_user_by_email_or_phone.return_value = None
        ...
        mock_user_repo.get_user_by_email_or_phone.assert_awaited_once_with(
            conn=ANY, email="nonexistent@example.com"
        )
...
class TestVerifyCodeAndResetPassword:
    ...
    async def test_reset_password_success(
        ...
        result = await auth_service.verify_code_and_reset_password(
            code="123456", new_password="new_strong_password"
        )
    ...
    async def test_reset_password_invalid_code(self, auth_service):
        ...
        with pytest.raises(HTTPException) as excinfo:
            await auth_service.verify_code_and_reset_password(
                code="invalidcode", new_password="any_password"
            )
