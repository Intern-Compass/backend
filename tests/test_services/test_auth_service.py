...
class TestCreateUnverifiedNewUser:
    ...
    async def test_create_new_user_success(
        self, auth_service, mock_user_repo, mock_intern_repo, mock_supervisor_repo, mock_code_repo
    ):
        ...
        # Verify the result
        mock_user_repo.get_user_by_email_or_phone.assert_awaited_once_with(conn=auth_service.session, email=new_user_data.email, phone_number=new_user_data.phone_number)
        mock_user_repo.create_new_user.assert_awaited_once_with(conn=auth_service.session, new_user=new_user_data, skill_repo=auth_service.skill_repo)
    ...
    async def test_create_new_intern_success(
        ...
        # Verify the result
        mock_user_repo.get_user_by_email_or_phone.assert_awaited_once_with(conn=auth_service.session, email=new_intern_data.email, phone_number=new_intern_data.phone_number)
        mock_user_repo.create_new_user.assert_awaited_once_with(conn=auth_service.session, new_user=new_intern_data, skill_repo=auth_service.skill_repo)
        mock_intern_repo.create_new_intern.assert_awaited_once_with(conn=auth_service.session, new_intern=new_intern_data, user=created_user_mock)
    ...
    async def test_create_new_supervisor_success(
        ...
        mock_user_repo.get_user_by_email_or_phone.return_value = None
        mock_user_repo.create_new_user.return_value = created_user_mock
        mock_supervisor_repo.create_new_supervisor.return_value = created_supervisor_mock
        # Call the service method
        result = await auth_service.create_unverified_new_user(new_supervisor_data)

        # Verify the result
        mock_user_repo.get_user_by_email_or_phone.assert_awaited_once_with(conn=auth_service.session, email=new_supervisor_data.email, phone_number=new_supervisor_data.phone_number)
        mock_user_repo.create_new_user.assert_awaited_once_with(conn=auth_service.session, new_user=new_supervisor_data, skill_repo=auth_service.skill_repo)
        mock_supervisor_repo.create_new_supervisor.assert_awaited_once_with(conn=auth_service.session, new_supervisor=new_supervisor_data, user=created_user_mock)
    ...
    async def test_existing_unverified_user(
        ...
        mock_code_repo.upsert_code_with_user_id.assert_awaited_once_with(conn=auth_service.session, user_id=existing_unverified_user.id, value=ANY)
        assert result == {"detail": "Verification code sent to email"}
    ...
    async def test_existing_verified_user_raises_exception(
        ...
        assert "User already exists with specified email or phone number. Please log in" in excinfo.value.detail
...
class TestVerifyUser:
    ...
    async def test_verify_user_success_for_all_types(
        ...
        mock_code_repo.delete_code.assert_awaited_with(conn=auth_service.session, value="123456")
    ...
    async def test_verify_user_raises_exception_for_invalid_code(self, auth_service, mock_code_repo):
...
class TestLogin:
    ...
    async def test_login_success_for_all_types(
        ...
        result = await auth_service.login(username="test@example.com", password="correct_password")

        mock_user_repo.get_user_by_email_or_phone.assert_awaited_once_with(
            conn=auth_service.session, email="test@example.com"
        )
        mock_password_check.assert_called_once_with(existing_user.password, "correct_password")
        mock_access_token.new.assert_called_once()
        user_to_login_arg = mock_access_token.new.call_args[1]['user'] # Access the first positional argument passed to the mock_generate_token function
        assert isinstance(user_to_login_arg, expected_out_model)
        assert result == {"access_token": "fake.access.token", "token_type": "Bearer"}

    async def test_login_fails_for_non_existent_user(self, auth_service, mock_user_repo):
        mock_user_repo.get_user_by_email_or_phone.return_value = None

        with pytest.raises(HTTPException) as exc_info:
            await auth_service.login(username="nouser@example.com", password="any_password")
    ...
    async def test_login_fails_for_incorrect_password(self, mock_password_check, auth_service, mock_user_repo):
        existing_user = create_mock_user(verified=True)
        mock_user_repo.get_user_by_email_or_phone.return_value = existing_user

        with pytest.raises(HTTPException) as exc_info:
            await auth_service.login(username="test@example.com", password="wrong_password")
    ...
    @patch("src.services.auth_service.password_is_correct", return_value=True)
    async def test_login_fails_for_unverified_user(self, mock_password_check, auth_service, mock_user_repo):
        unverified_user = create_mock_user(verified=False)
        mock_user_repo.get_user_by_email_or_phone.return_value = unverified_user

        with pytest.raises(HTTPException) as exc_info:
            await auth_service.login(username="unverified@example.com", password="correct_password")
...
class TestRequestResetPassword:
    ...
    async def test_request_reset_password_for_existing_user(
        self, mock_password_reset_token, auth_service, mock_user_repo, mock_background_tasks
    ):
        existing_user = create_mock_user(verified=True)
        mock_user_repo.get_user_by_email_or_phone.return_value = existing_user
        ...
        mock_background_tasks.add_task.assert_called_once()
        assert "If this email exists, a password reset email will be sent." in response["detail"]
    ...
    async def test_request_reset_password_for_non_existent_user(
        ...
        mock_user_repo.get_user_by_email_or_phone.assert_awaited_once_with(conn=auth_service.session, email="nouser@example.com")
...
class TestVerifyCodeAndResetPassword:
    ...
    async def test_reset_password_success(
        ...
        mock_background_tasks.add_task.assert_called_once()
        assert "Your password has been reset successfully, please proceed to log in" in response["detail"]
