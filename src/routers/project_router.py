...
async def get_tasks_by_project_id(
    ...
    return await project_service.get_tasks_by_project_id(
        project_id=UUID(project_id), supervisor_id=UUID(supervisor.supervisor_id)
    )
...
async def mark_task_as_completed(
    ...
    return await task_service.mark_task_as_completed(
        task_id=task_id, supervisor_id=supervisor.supervisor_id
    )
