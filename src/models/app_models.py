...
class Division(Base):
    ...
    supervisors: Mapped[List[Supervisor]] = relationship(
        "Supervisor", back_populates="division"
    )
...
class InternSkill(Base):
    ...
    intern_id: Mapped[UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("intern.id"), primary_key=True
    )
...
class TaskSkill(Base):
    ...
    task_id: Mapped[UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("task.id"), primary_key=True
    )
    skill_id: Mapped[UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("skill.id"), primary_key=True
    )
...
class SupervisorSkill(Base):
    ...
    supervisor_id: Mapped[UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("supervisor.id"), primary_key=True
    )
    skill_id: Mapped[UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("skill.id"), primary_key=True
    )
