...
class TodoService:
    ...
    async def get_todos(
        self, user_id: UUID, is_done: bool | None
    ) -> list[TodoOutModel]:
        async with self.session.begin():
            intern = await self.intern_repo.get_intern_by_user_id(
    ...
    async def complete_todo(self, todo_id: UUID, intern_id: UUID) -> dict:
        async with self.session.begin():
            existing_todo: Todo = await self.todo_repo.get_todo_by_id(
            ...
            todo_data = TodoUpdateModel(done=True)
