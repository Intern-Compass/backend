...
class AuthService:
    ...
    async def create_unverified_new_user(self, new_user: UserInModel|InternInModel) -> dict[str, str]:
        async with self.session.begin():  # Transactional, for atomicity
            ...
            if existing_user:
                ...
                unverified_user: User = (
                    ...
                    await self.user_repo.create_new_user(
                        conn=self.session,
                        new_user=new_user
                    )
                )
    ...
    async def verify_user(self, code: str) -> dict[str, str]:
        async with self.session.begin(): #Transactional, for atomicity
            verification_code: VerificationCode = await self.code_repo.get_code(conn=self.session, value=code)
    ...
    async def login(self, username: str, password: str, res: Response):
        existing_user: User = await self.user_repo.get_user_by_email_or_phone(conn=self.session, email=username)
