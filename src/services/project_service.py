...
class ProjectService:
    ...
    async def create_project(
        self, project_data: ProjectInModel, supervisor_id: UUID
    ) -> ProjectOutModel:
    ...
    async def add_task_to_project(
        self, project_id: UUID, supervisor_id: UUID, task_data: TaskInModel
    ) -> TaskOutModel:
        async with self.session.begin():
            project: Project | None = await self.project_repo.get_project_by_id(
            ...
            if project.supervisor_id != supervisor_id:
                raise HTTPException(
                    ...
                    detail="Supervisor not authorized to add task to this project",
                )
            # Create new task
    ...
    async def get_tasks_by_project_id(
        self, project_id: UUID, supervisor_id: UUID
    ) -> list[TaskOutModel]:
        async with self.session.begin():
            ...
            if project.supervisor_id != supervisor_id:
                raise HTTPException(
                    status_code=HTTP_403_FORBIDDEN,
                    detail="Supervisor not authorized to view this project",
                )
    ...
    async def get_all_projects_created_by_supervisor(self, supervisor_id: UUID):
        async with self.session.begin():
            projects: list[
                Project
            ] = await self.project_repo.get_all_projects_by_supervisor_id(
                conn=self.session, supervisor_id=supervisor_id
            )
